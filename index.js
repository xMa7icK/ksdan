...
const start = (kill = new Client()) => {
    console.log(color('\n[DEV]', 'red'), color('- +569 7652 7581 <-> https://chat.whatsapp.com/GFmu1qRL5QZEehGPuT1vPa'))
	console.log(color('[Íris]', 'red'), color('[Matias] READY... Ya puedes usar coamndos\n'))
		...
		kill.onGlobalParticipantsChanged(async (event) => {
			...
			try {
				if (event.action == 'add') {
					...
					await kill.sendTextWithMentions(event.chat, `SE FUEEEEEEEE UNA PERRAAA🎶🎵...... @${event.who.replace('@c.us', '')} ... \nF. ~Nadie l@ extrañara~ Sigamos en lo que estabamos.🙂`)
				}
			} catch (err) {
				console.log(err)
			}
        ...
        kill.onAddedToGroup(async (chat) => {
			...
			if (chat.groupMetadata.participants.includes(config.owner)) {
            	...
            	await kill.sendText(chat.id, `Un nuevo grupo, 7u7! 😃\nLástima que no tenga el requisito minimo de putas, que es al menos ${config.memberLimit} putas. Tú tienes ${totalMem}, cuando reunas las putas necesarias vuelve a solicitar mis servicios! 😉`)
        ...
        kill.onIncomingCall(async (call) => {
            await kill.sendText(call.peerJid, `¡Que pena! No quiero hablar con una puta! 😊\nTe bloqueé para evitar discuciones con mi pareja, contacta al dueño wa.me/${config.owner.replace('c.us', '')} para desbloquear. 👋`)
